...
; 
; Version 1.0
; Version 1.1 Updated to work with Gimp 2.10
;             Gimp 2.10 doesn't allow rotation by 0 degrees (stupidly!)
;             so had to add an 'if' statement for that case.  It also
;             doesn't permit rotation by 360 degrees, etc; so need a
;             modulo to take care of that
; ==========================================================================
...
(define (better-plug-in-rotate interactive inImage cpyLayer 
			       rotateAngle rotWholeImage)
  ;; substitute for plug-in-rotate that allows rotAngle to be any non-negative integer
  (let *
      (
       (rotAngle (modulo rotateAngle 4))
       )
    (if (> rotAngle 0) 
	(plug-in-rotate interactive inImage cpyLayer 
			rotAngle rotWholeImage)
	)
    )
  );; end define better-plug-in-rotate
...
(define (script-fu-fibonacci-spiral inImage inLayer 
  ...
  (let* 
    ...
    (while (> (- currentSize nextSize) 0)
	   ...
	   (set! rotAngle (modulo (* rotateAngle count) 4))
	   (if (> rotAngle 0) ;; stupid plug-in-rotate wont take 0
...
(script-fu-menu-register "script-fu-fibonacci-spiral"
